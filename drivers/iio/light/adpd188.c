			      ...
			      unsigned int *rx_val)
{
	...
	if ((dev->phy_opt != ADPD188_I2C) || (dev_id == 0)) {
		if (rx_val)
			ops->reg_read(dev->bus, reg, rx_val);
		else
			ops->reg_write(dev->bus, reg, tx_val);
	} else {
		if (rx_val)
			ops->i2c_sub_read(dev->bus,
						 (i2c->addr + dev_id),
						 reg, rx_val);
		else
			ops->i2c_sub_write(dev->bus,
						  (i2c->addr + dev_id),
						  reg, tx_val);
	}
			     ...
			     int val, int val2, long mask)
{
	...
	adpd188_all_set_device_mode(data, STANDBY);
...
static const char * const adpd188_modes_ascii[] =
 	{"STANDBY", "PROGRAM", "NORMAL"};
